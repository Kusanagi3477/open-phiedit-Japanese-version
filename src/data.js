...
function official_to_rpe(off) {
  ...
  for (let i = 0; i < off.judgeLineList.length; i++) {
    ...
    for (let tmp of [
      [1, off.judgeLineList[i].notesAbove],
      [2, off.judgeLineList[i].notesBelow],
    ]) {
      for (let j of tmp[1]) {
      ...
      if (key != "") {
        ...
        function add_line(n) {
          while (res.judgeLineList.length <= n) {
            let line = new_judge_line();
            for (let j in line.eventLayers[0])
              line.eventLayers[0][j].length = 0;
            res.judgeLineList.push(line);
          }
        }
        if (op == "bp" && args.length == 2) {
          ...
          let id = Number(args[0]);
          add_line(id);
          res.judgeLineList[id].notes.push(
            note_compress({
              above: Number(args[4]),
              alpha: 255,
              endTime: tob(Number(args[2])),
...
function large_json_parse(e) {
  ...
  if (e.str[e.i] === 123) {
    ...
    e.i++;
    return result;
  } else if (e.str[e.i] === 110) {
    // null
    e.i += 4;
    return null;
  } else if (e.str[e.i] === 116) {
    // true
    e.i += 4;
    return true;
  } else if (e.str[e.i] === 102) {
    // false
    e.i += 5;
    return false;
  } else if (e.str[e.i] === 34) {
    // string
    var result = "";
    e.i++; // 跳过双引号
    ...
    // number
    var numStr = "";
    while (45 <= e.str[e.i] && e.str[e.i] <= 57)
      numStr += String.fromCharCode(e.str[e.i++]);
              ...
              if (json.formatVersion) {
                ...
                for (let i = 0; i < all_data.judgeLineList.length; i++) {
                  if (all_data.judgeLineList[i].notes) {
                    for (
                      let j = 0;
                      j < all_data.judgeLineList[i].notes.length;
                      j++
                    ) {
                ...
                if (ispec) {
                  ...
                  for (let i = 1; i <= n; i++) {
                    ...
                    let ed =
                      type === 3
                        ? st +
                          Math.floor((parseInt(tmp[3]) / 66.6) * (bpm / 60))
                        : st;
                    let note = {
                      type: type,
                      startTime: [
