...
var n = 114514,
  heng = 4,
  shu = 7,
  hi = 200; // 小节数，横着分几格，竖着分几格，每小节高度
var selection = []; // 当前选中的音符
var selection_ev = []; // 当前选中的事件，格式：[编号，类型]
var put_hold_st = null,
  put_hold_x; // 放置 Hold 音符用
var put_ev_st = null,
  put_ev_x; // 放置事件用
...
function change_line(i) {
  ...
  if (!lines.children[i]) {
    console.warn("line index not found:", i);
    return;
  }

  selected.innerHTML = lines.children[i].innerHTML;

  if (!all_data.judgeLineList[i]) {
    console.warn("judgeLineList empty");
    return;
  }
...
var notecontrol = {
  /*
	...
	*/
  add: function (type, time, x, auto_update = 1) {
    // 注意：调用此函数会自动更新渲染缓存，除非更改参数 auto_update
    /*
  ...
  addhold: function (st, ed, x, auto_update = 1) {
    // 注意：调用此函数会自动更新渲染缓存，除非更改参数 auto_update
    /*
		type: 1tap 2hold 3flick 4drag
		*/
  ...
  update: function () {
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      let y =
        -450 + hi * (note.startTime[0] + note.startTime[1] / note.startTime[2]);
      let h = (note.positionX / 5).toFixed(0) * 10000 + (y / 5).toFixed(0);
      if (note.type == 2) {
        let y2 =
          note.type == 2
            ? -450 + hi * (note.endTime[0] + note.endTime[1] / note.endTime[2])
            : y;
  ...
  render: function (x1, y1, x2, y2) {
    ...
    function draw(t, x, y, sz, ang /* canvas 坐标系 */) {
      let w = 1089 * sz,
        h = [100, null, 200, 60][t - 1] * sz;
      if (x + w >= 0 && x <= 1600 && y + h >= 0 && y <= 900) {
        // 屏幕内的才绘制
    ...
    for (let i = 0; i < this.render_cache.length; i++) {
      if (this.render_cache[i].t == 2) {
        ...
        drawhold(
          x1 + (x2 - x1) * ((this.render_cache[i].x + 675) / 1350),
          y2 - ((st + 450) / 900) * (y2 - y1),
          y2 - ((ed + 450) / 900) * (y2 - y1),
          selection.includes(this.render_cache[i].id) ? 0.13 : 0.1,
        );
      }
    }
    if (put_hold_st != null) {
    ...
    for (let i = 0; i < this.render_cache.length; i++) {
      if (this.render_cache[i].t != 2) {
        ...
        draw(
          this.render_cache[i].t,
          x1 + (x2 - x1) * ((this.render_cache[i].x + 675) / 1350),
          y2 - ((y + 450) / 900) * (y2 - y1),
          selection.includes(this.render_cache[i].id) ? 0.13 : 0.1,
          this.render_cache[i].ang,
        );
      }
    }
  },
  put_calc: (x1, y1, x2, y2, tx, ty, _shu = shu) => {
    // 根据鼠标坐标，推出被点击音符的位置
    ...
    return [
      Math.round((x - 675) / (1350 / _shu)) * (1350 / _shu) + 675,
      [a, b, heng],
    ];
  ...
  hold_calc: (x, yst, yed, x1, y1, x2, y2) => {
    // 坐标 x，时间范围 (yst-yed)，渲染区间 (x1,y1) 到 (x2,y2)，求出到鼠标的距离的平方
    yst += rdelta;
    yed += rdelta;
    let tx = x1 + (x2 - x1) * ((x + 675) / 1350);
    // 长条下面的 y（开始触发），上面的 y（结束触发），tyst > tyed
    let tyst = y2 - ((yst + 450) / 900) * (y2 - y1),
      tyed = y2 - ((yed + 450) / 900) * (y2 - y1);
    let dis = 0;
    if (mousedata.y > tyed && mousedata.y < tyst) {
      dis = (tx - mousedata.x) * (tx - mousedata.x);
    } else if (mousedata.y < tyed) {
      dis =
        (tx - mousedata.x) * (tx - mousedata.x) +
        4 * (tyed - mousedata.y) * (tyed - mousedata.y);
    } else {
      dis =
        (tx - mousedata.x) * (tx - mousedata.x) +
        4 * (tyst - mousedata.y) * (tyst - mousedata.y);
    }
    return dis;
  },
  check_select: function (x1, y1, x2, y2, minn = Infinity) {
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      let dis =
        (tx - mousedata.x) * (tx - mousedata.x) +
        4 * (ty - mousedata.y) * (ty - mousedata.y);
    ...
    if (minn > 6000) {
      for (let i = 0; i < notes.length; i++) {
        ...
        let yst =
          -450 +
          hi * (note.startTime[0] + note.startTime[1] / note.startTime[2]);
        let yed =
          -450 + hi * (note.endTime[0] + note.endTime[1] / note.endTime[2]);
...
// ============================================================================================
var eventcontrol = {
  add: (typex, st, ed, stval = 0, edval = 0) => {
    // 添加事件
    let t = [
      "moveXEvents",
      "moveYEvents",
  ...
  ex_add: (typex, st, ed, stval = 0, edval = 0) => {
    // 添加扩展事件
    let t = [
      "scaleXEvents",
      "scaleYEvents",
  ...
  render: function (x1, y1, x2, y2) {
    function draw(x, y, y2, sz /* canvas 坐标系 */, val1, val2) {
      // val1 是上面的数字 val2 是下面的数字
      ...
      if (val2 != null)
        ctx.fillText(
          val2.toFixed(settingscontrol.settings.decimal),
          x,
          y2 - 18,
        );
    }
    ...
    for (let type of all_evs_list) {
      ...
      for (let i = 0; i < ev.length; i++) {
        let st =
          -450 +
          hi * (ev[i].startTime[0] + ev[i].startTime[1] / ev[i].startTime[2]);
        st += rdelta;
        let ed =
          -450 + hi * (ev[i].endTime[0] + ev[i].endTime[1] / ev[i].endTime[2]);
        ed += rdelta;
        let f = 0;
        ...
        if (type == "colorEvents")
          draw(
            ...
            y2 - ((st + 450) / 900) * (y2 - y1),
            f ? 0.13 : 0.1,
            ev[i].end[0] * 65536 + ev[i].end[1] * 256 + ev[i].end[2],
            ev[i].start[0] * 65536 + ev[i].start[1] * 256 + ev[i].start[2],
          );
  ...
  getval_typex: (typex, time) => {
    // 注意区分 typex 和 typename
    let typename = [
      "moveXEvents",
      "moveYEvents",
      "rotateEvents",
...
var sidebarcontrol = {
  initedit: () => {
    ...
    $("edit-time").addEventListener("change", () => {
      if (selection.length == 1) {
        ...
        if (is_beat(s)) {
          ...
          if (note != 2) {
            (($("edit-time2").value = $("edit-time").value),
    ...
    $("edit-time2").addEventListener("change", () => {
      if (selection.length == 1) {
        ...
        if (t(s)) {
          ...
          note.startTime = [Number(s[0]), Number(s[1]), Number(s[2])];
          if (note != 2) {
            (($("edit-time").value = $("edit-time2").value),
    ...
    $("edit2-ease").addEventListener("change", () => {
      if (selection_ev.length > 0) {
        if (
          isNaN(Number($("edit2-ease").value)) ||
          (selection_ev[0][1] == "speedEvents" && $("edit2-ease").value != "1")
        )
          $("edit-x").value =
            evs[evs_layer][selection_ev[0][1]][selection_ev[0][0]].easingType;
        else {
          for (
            let i = 0, v = Number($("edit2-ease").value);
            i < selection_ev.length;
            i++
          ) {
            evs[evs_layer][selection_ev[0][1]][selection_ev[0][0]].easingType =
              v;
    ...
    $("edit2-time").addEventListener("change", () => {
      if (selection_ev.length == 1) {
        let e = (evs_layer == "ex" ? now_line.extended : evs[evs_layer])[
          selection_ev[0][1]
        ][selection_ev[0][0]];
    ...
    function check_val(data, type) {
      // 传入文本框的原始数据，根据事件类型自动转换
      if (type == "colorEvents") {
        let tmp = data.split(",");
        if (
          tmp.length == 3 &&
          ...
          return [Number(tmp[0]), Number(tmp[1]), Number(tmp[2])];
        else return null;
      } else if (type == "textEvents") {
        return data;
      } else {
    ...
    $("edit4-impl").addEventListener("click", () => {
      ...
      selection.sort((a, b) => {
        return cmp(
          note_extract(notes[a]).startTime,
          note_extract(notes[b]).startTime,
        )
          ? -1
          : 1;
      });
      if (t == "line") {
        if (op == "set") {
          for (let i = 0; i < selection.length; i++) {
            ...
            v = Math.floor(v);
            if (v < 0) v = 0;
            if (v >= all_data.judgeLineList.length)
              v = all_data.judgeLineList.length - 1;
        ...
        for (let i = 0; i < selection.length; i++) {
          let v = v1 + (v2 - v1) * calcease(ease, i / (selection.length - 1));
          if (!isNaN(Number(cycle[i % cycle.length])))
            v *= cycle[i % cycle.length];
          let note = note_extract(notes[selection[i]]);
          switch (op) {
            case "add":
              note[t] += v;
            ...
            case "mul":
              ...
              break;
            case "max":
              note[t] = Math.max(note[t], v);
              break;
            case "min":
              note[t] = Math.min(note[t], v);
              break;
            case "flip":
              note[t] = 2 * v - note[t];
              break;
  ...
  edit_update: () => {
    ...
    if (selection.length == 1) {
      ...
      $("edit-time2").value =
        note.endTime[0] + ":" + note.endTime[1] + "/" + note.endTime[2];
      ...
      show(2);
      if (evs_layer == "ex")
        var e = now_line.extended[selection_ev[0][1]][selection_ev[0][0]];
      ...
      $("edit2-time").value =
        e.startTime[0] + ":" + e.startTime[1] + "/" + e.startTime[2];
      $("edit2-time2").value =
        e.endTime[0] + ":" + e.endTime[1] + "/" + e.endTime[2];
...
var settingscontrol = {
  load: () => {
    ...
    for (let i = 0; i < bpm.length; i++) {
      ...
      e2.addEventListener("change", () => {
        ...
        if (is_beat(s))
          bpm[i].startTime = [Number(s[0]), Number(s[1]), Number(s[2])];
        else
          e2.value =
            bpm[i].startTime[0] +
            ":" +
  ...
  init: () => {
    ...
    $("s-shu").addEventListener("change", () => {
      if (isNaN(Number($("s-shu").value)) || Number($("s-shu").value) > 100)
        $("s-shu").value = shu;
      else shu = Number($("s-shu").value);
    });
    $("s-heng").addEventListener("change", () => {
      if (isNaN(Number($("s-heng").value)) || Number($("s-heng").value) > 100)
        $("s-heng").value = heng;
    ...
    $("bpmlist-add").addEventListener("click", () => {
      bpm.push({
        startTime: bpm[bpm.length - 1].startTime,
        bpm: bpm[bpm.length - 1].bpm,
      });
      settingscontrol.load();
    });
  },
...
function main() {
  ...
  if (playing == 1) {
    ...
    if (mousedata.sel_rect.x1 != null) {
      ...
      ctx.fillRect(
        mousedata.sel_rect.x1,
        y1,
        mousedata.x - mousedata.sel_rect.x1,
        mousedata.y - y1,
      );
    }
  }
  ...
  if (all_data.judgeLineList.length != r_last_lines_length) {
    ...
    for (let i = 0; i < all_data.judgeLineList.length; i++) {
      ...
      op.innerHTML = `<div class="option-judgeline" data-i="${i}">
					[${i}]
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    nodes[i].innerText =
      "(" +
      renderer
        .getval(all_data.judgeLineList[id], "moveXEvents", -rdelta / hi)
        .toFixed(0) +
      "," +
      renderer
        .getval(all_data.judgeLineList[id], "moveYEvents", -rdelta / hi)
        .toFixed(0) +
      ")";
  }
  ...
  for (let i = 0; i < nodes.length; i++) {
    let id = nodes[i].parentNode.dataset.i;
    nodes[i].innerText = renderer
      .getval(all_data.judgeLineList[id], "rotateEvents", -rdelta / hi)
      .toFixed(0);
  }
...
var playercontrol = {
  ...
  updateTime: function () {
    rdelta =
      -hi *
      sec_to_beat(
        this.startChartTime + (performance.now() - this.startTime) / 1000,
      );
  },
};
// 复制粘贴
...
var clipboardcontrol = {
  ...
  copy: function () {
    if (selection.length > 0) {
      let arr = [];
      for (let i = 0; i < selection.length; i++)
        arr.push(note_extract(notes[selection[i]]));
      ...
      for (let i = 0; i < selection_ev.length; i++)
        arr.push([
          selection_ev[i][1],
          evs[evs_layer][selection_ev[i][1]][[selection_ev[i][0]]],
        ]);
      this.copy_str(JSON.stringify(arr));
    }
  },
...
// 操作：键盘、其他
// ============================================================================================
function put_qwer(key) {
  // 识别 q,w,e,r 键；这里专门搞一个函数出来，是为了适配手机端！
  if (key == "q") {
    notecontrol.put(
      nrr.x1,
      nrr.y1,
    ...
    if (edit_event && mousedata.x > (nrr.x2 + evrr.x1) / 2) {
        ...
        if (cmp(put_ev_st, tmp[1])) {
          if (evs_layer == "ex") {
            ...
            eventcontrol.ex_add(
              ...
              put_ev_st,
              tmp[1],
              eventcontrol.ex_getval_typex(tmp[0], put_ev_st),
              put_ev_x == -135 ? [255, 255, 255] : put_ev_x == 135 ? "" : 0,
            );
          } else {
            eventcontrol.add(
              put_ev_x,
              put_ev_st,
              tmp[1],
              eventcontrol.getval_typex(tmp[0], put_ev_st),
            );
          }
        }
        put_ev_st = null;
      ...
      if (put_hold_st != null) {
        if (cmp(put_hold_st, tmp[1]))
          notecontrol.addhold(put_hold_st, tmp[1], put_hold_x);
        put_hold_st = null;
      } else if (tmp != null) {
...
function delete_selection() {
  ...
  // 删除事件：
  for (let i = 0; i < selection_ev.length; i++) {
    (evs_layer == "ex" ? now_line.extended : evs[evs_layer])[
      selection_ev[i][1]
    ].splice(selection_ev[i][0], 1);
...
$("mobile-phone-delete").addEventListener("click", () => {
  // 等价于按下 delete
  delete_selection();
});
$("mobile-phone-play").addEventListener("click", () => {
  // 等价于按下空格
...
$("eventlayer-add").addEventListener("click", () => {
  // 添加事件层级
  evs.push({
    alphaEvents: [],
    moveXEvents: [],
...
$("eventlayer-del").addEventListener("click", () => {
  // 删除事件层级
  if (evs.length > 1) {
    let e = evs[evs.length - 1];
    let cnt =
      e.alphaEvents.length +
      e.moveXEvents.length +
      e.moveYEvents.length +
      e.rotateEvents.length +
      e.speedEvents.length;
    if (
      cnt == 0 ||
      "yes" ==
        window.prompt("最后一个事件层级已经有事件了，是否强制删除？是输入 yes")
    )
      evs.pop();
  }
});
...
$("cvs").addEventListener("mousemove", function (event) {
  ...
  if (mousedata.drag != null) {
    ...
    if (
      s[0] != notes[mousedata.drag].positionX ||
      !cmparray(s[1], notes[mousedata.drag].startTime)
    ) {
      ...
      if (event.shiftKey) {
        if (mousedata.sel_rect.x1 != null) {
          ...
          for (let i = 0; i < notes.length; i++) {
            ...
            let note = note_extract(notes[i]);
            let tx =
              nrr.x1 + (nrr.x2 - nrr.x1) * ((note.positionX + 675) / 1350); // 转换到 canvas 坐标系
            if (note.type == 2) {
              if (
                x1 <= tx &&
                tx <= x2 &&
                ((y1 <= to(note.startTime) && to(note.startTime) <= y2) ||
                  (y1 <= to(note.endTime) && to(note.endTime) <= y2))
              ) {
                selection.push(i);
              ...
              if (
                x1 <= tx &&
                tx <= x2 &&
                y1 <= to(note.startTime) &&
                to(note.startTime) <= y2
              ) {
                selection.push(i);
                selection_ev.length = 0;
              }
        ...
        if (p != null && notes[p].type != 2) {
          mousedata.drag = p;
          if (selection.length == 0) mousedata.dragtype = "single";
    ...
    if (!mousedata.moved) {
      ...
      if (mode != "computer") {
        ...
        if (edit_event && mousedata.x > (nrr.x2 + evrr.x1) / 2) {
          ...
          selection = [];
          if (control_down == 0) {
            if (selection_ev.length == 1 && cmparray(selection_ev[0], p))
              selection_ev = [];
            else selection_ev = [p];
          } else {
...
$("fillnotes-open").addEventListener("click", () => {
  ...
  if (selection.length == 1) {
    $("fillnotes-time").value =
      note0.startTime[0] + ":" + note0.startTime[1] + "/" + note0.startTime[2];
...
$("fillnotes").addEventListener("click", () => {
  ...
  // x 坐标的区间
  let x = parseFloat($("fillnotes-x").value),
    x2 = parseFloat($("fillnotes-x2").value);
  if (isNaN(x) || isNaN(x2)) return;

  let t = parseInt($("fillnotes-t").value);
  let ease = $("fillnotes-ease").value;

  if ($("fillnotes-flag").checked) {
    // 包含边界？
    for (let i = st; cmp(i, ed); i = add(i, density))
      notecontrol.add(
        ...
        x == x2
          ? x
          : x + (x2 - x) * calcease(ease, div(sub(i, st), sub(ed, st))),
        0,
      );
  } else {
...
function autosaveTrigger() {
  ...
  window.saveDelay = setTimeout(saveChartToDB, 300);
}
